...
namespace {
typedef int16_t HNode;

static const HNode huffmanTab0[] = {
    0,
};

static const HNode huffmanTab1[] = {
    -5, -3, -1, 17, 1, 16, 0,
};

static const HNode huffmanTab2[] = {
    -15, -11, -9, -5, -3, -1, 34, 2, 18, -1, 33, 32, 17, -1, 1, 16,
    0,
};

static const HNode huffmanTab3[] = {
    -13, -11, -9, -5, -3, -1, 34, 2, 18, -1, 33, 32, 16, 17, -1, 1,
    0,
};

static const HNode huffmanTab5[] = {
    -29, -25, -23, -15, -7, -5, -3, -1, 51, 35, 50, 49, -3, -1, 19, 3,
    -1, 48, 34, -3, -1, 18, 33, -1, 2, 32, 17, -1, 1, 16, 0,
};

static const HNode huffmanTab6[] = {
    -25, -19, -13, -9, -5, -3, -1, 51, 3, 35, -1, 50, 48, -1, 19, 49,
    -3, -1, 34, 2, 18, -3, -1, 33, 32, 1, -1, 17, -1, 16, 0,
};

static const HNode huffmanTab7[] = {
...
static const HNode huffmanTab8[] = {
...
static const HNode huffmanTab9[] = {
...
static const HNode huffmanTab10[] = {
...
static const HNode huffmanTab11[] = {
...
static const HNode huffmanTab12[] = {
...
static const HNode huffmanTab13[] = {
...
static const HNode huffmanTab15[] = {
...
static const HNode huffmanTab16[] = {
...
static const HNode huffmanTab24[] = {
...
static const HNode huffmanTabC0[] = {
    -29, -21, -13, -7, -3, -1, 11, 15, -1, 13, 14, -3, -1, 7, 5, 9,
    -3, -1, 6, 3, -1, 10, 12, -3, -1, 2, 1, -1, 4, 8, 0,
};

static const HNode huffmanTabC1[] = {
    -15, -7, -3, -1, 15, 14, -1, 13, 12, -3, -1, 11, 10, -1, 9, 8,
    -7, -3, -1, 7, 6, -1, 5, 4, -3, -1, 3, 2, -1, 1, 0,
};

struct TableInfo { const HNode* tree; uint16_t length; uint8_t linbits; };

static const TableInfo tables[34] = {
    { huffmanTab0, (uint16_t)(sizeof(huffmanTab0)/sizeof(huffmanTab0[0])), 0 },
    { huffmanTab1, (uint16_t)(sizeof(huffmanTab1)/sizeof(huffmanTab1[0])), 0 },
    { huffmanTab2, (uint16_t)(sizeof(huffmanTab2)/sizeof(huffmanTab2[0])), 0 },
    { huffmanTab3, (uint16_t)(sizeof(huffmanTab3)/sizeof(huffmanTab3[0])), 0 },
    { huffmanTab0, (uint16_t)(sizeof(huffmanTab0)/sizeof(huffmanTab0[0])), 0 },
    { huffmanTab5, (uint16_t)(sizeof(huffmanTab5)/sizeof(huffmanTab5[0])), 0 },
    { huffmanTab6, (uint16_t)(sizeof(huffmanTab6)/sizeof(huffmanTab6[0])), 0 },
    { huffmanTab7, (uint16_t)(sizeof(huffmanTab7)/sizeof(huffmanTab7[0])), 0 },
    { huffmanTab8, (uint16_t)(sizeof(huffmanTab8)/sizeof(huffmanTab8[0])), 0 },
    { huffmanTab9, (uint16_t)(sizeof(huffmanTab9)/sizeof(huffmanTab9[0])), 0 },
    { huffmanTab10, (uint16_t)(sizeof(huffmanTab10)/sizeof(huffmanTab10[0])), 0 },
    { huffmanTab11, (uint16_t)(sizeof(huffmanTab11)/sizeof(huffmanTab11[0])), 0 },
    { huffmanTab12, (uint16_t)(sizeof(huffmanTab12)/sizeof(huffmanTab12[0])), 0 },
    { huffmanTab13, (uint16_t)(sizeof(huffmanTab13)/sizeof(huffmanTab13[0])), 0 },
    { huffmanTab0, (uint16_t)(sizeof(huffmanTab0)/sizeof(huffmanTab0[0])), 0 },
    { huffmanTab15, (uint16_t)(sizeof(huffmanTab15)/sizeof(huffmanTab15[0])), 0 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 1 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 2 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 3 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 4 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 6 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 8 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 10 },
    { huffmanTab16, (uint16_t)(sizeof(huffmanTab16)/sizeof(huffmanTab16[0])), 13 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 4 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 5 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 6 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 7 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 8 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 9 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 11 },
    { huffmanTab24, (uint16_t)(sizeof(huffmanTab24)/sizeof(huffmanTab24[0])), 13 },
    { huffmanTabC0, (uint16_t)(sizeof(huffmanTabC0)/sizeof(huffmanTabC0[0])), 0 },
    { huffmanTabC1, (uint16_t)(sizeof(huffmanTabC1)/sizeof(huffmanTabC1[0])), 0 },
};

static bool decodeTree(MP3BitReader &br, const TableInfo &t, uint16_t &symbol) {
    if (!t.tree || t.length == 0) return false;
    if (t.length == 1) { symbol = (uint16_t)t.tree[0]; return true; }
    int idx = 0;
    for (unsigned guard = 0; guard < 32; ++guard) {
        if (idx < 0 || idx >= (int)t.length) return false;
        const int v = t.tree[idx];
        if (v >= 0) { symbol = (uint16_t)v; return true; }
        if (!br.canRead(1)) return false;
        const bool bit = br.read(1) != 0;
        idx = bit ? (idx + 1 - v) : (idx + 1);
    }
    return false;
}

static int signValue(MP3BitReader &br, int v) {
    if (v == 0) return 0;
    if (!br.canRead(1)) return v;
    return br.read(1) ? -v : v;
}
...
bool decodePair(MP3BitReader &br, unsigned table, MP3HuffmanPair &out) {
    if (table > 31) return false;
    uint16_t sym = 0;
    if (!decodeTree(br, tables[table], sym)) return false;
    int x = (sym >> 4) & 15;
    int y = sym & 15;
    const uint8_t lb = tables[table].linbits;
    if (lb && x == 15) { if (!br.canRead(lb)) return false; x += (int)br.read(lb); }
    if (lb && y == 15) { if (!br.canRead(lb)) return false; y += (int)br.read(lb); }
    out.x = signValue(br, x);
    out.y = signValue(br, y);
    return true;
}

bool decodeQuad(MP3BitReader &br, unsigned table, MP3HuffmanQuad &out) {
    if (table < 32 || table > 33) return false;
    uint16_t sym = 0;
    if (!decodeTree(br, tables[table], sym)) return false;
    out.v = signValue(br, (sym >> 3) & 1);
    out.w = signValue(br, (sym >> 2) & 1);
    out.x = signValue(br, (sym >> 1) & 1);
    out.y = signValue(br, sym & 1);
    return true;
}
...
bool decodeGranule(MP3BitReader &br,
                   ...
                   unsigned &bitsUsed) {
    pairsDecoded = quadsDecoded = bitsUsed = 0;
    memset(spectral, 0, 576 * sizeof(int16_t));
    if (bigValues > 288) bigValues = 288;
    const size_t start = br.bitsRead();
    const size_t end = start + part23Bits;
    if (br.bitsLeft() < part23Bits) return false;

    for (unsigned pair = 0; pair < bigValues; ++pair) {
        /* region boundaries are MPEG spectral-line indices.  Each Huffman
           pair represents two spectral lines, so compare 2*pair here. */
        const unsigned line = pair * 2u;
        const unsigned table = (line < region1Start) ? table0
                             : (line < region2Start) ? table1 : table2;
        MP3HuffmanPair p;
        if (!decodePair(br, table, p)) return false;
        spectral[pair*2] = (int16_t)p.x;
        spectral[pair*2+1] = (int16_t)p.y;
        ++pairsDecoded;
        if (br.bitsRead() > end) return false;
    }

    while (br.bitsRead() < end && 2*pairsDecoded + 4*quadsDecoded < 576) {
        const size_t before = br.bitsRead();
        MP3HuffmanQuad q;
        if (!decodeQuad(br, count1Table, q)) {
            br.skip((unsigned)(end - br.bitsRead()));
            break;
        }
        const unsigned idx = 2*pairsDecoded + 4*quadsDecoded;
        spectral[idx] = (int16_t)q.v;
        spectral[idx+1] = (int16_t)q.w;
        spectral[idx+2] = (int16_t)q.x;
        spectral[idx+3] = (int16_t)q.y;
        ++quadsDecoded;
        if (br.bitsRead() <= before || br.bitsRead() > end) return false;
    }
    if (br.bitsRead() < end) br.skip((unsigned)(end - br.bitsRead()));
    bitsUsed = (unsigned)(br.bitsRead() - start);
    return true;
}
