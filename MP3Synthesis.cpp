...
/*
 * MPEG-1 Layer III 32-subband polyphase synthesis filterbank.
 *
 * Input:
 *     576 hybrid time-domain values = 32 subbands x 18 samples
 *
 * Output:
 *     576 signed 16-bit PCM samples
 *
 * IMPORTANT:
 *   - The synthesis state is persistent for each channel.
 *   - mp3SynthesisReset() is called ONCE at the beginning of a file.
 *   - It must NOT be called between MP3 frames or granules.
 *
 * Flow for every one of the 18 time slots:
 *
 *     32 subband samples
 *          |
 *     frequency inversion
 *          |
 *     32 -> 64 DCT synthesis matrix
 *          |
 *     persistent 1024-value V FIFO
 *          |
 *     512-value U selection
 *          |
 *     ISO 512-value D window
 *          |
 *     32 PCM samples
 *
 * The previous Stage17 implementation already contained the correct
 * general FIFO/window architecture, but the matrix stage was hidden inside
 * a generic 64x32 table.  This version makes the DCT32 synthesis operation
 * explicit and keeps the complete per-channel state in one place.
 */

/*
 * Persistent synthesis state.
 *
 * Two independent MPEG synthesis filterbanks are required for stereo.
 * gV[channel] is the 1024-value synthesis FIFO specified by Layer III.
 */
static float gV[2][1024];

/*
 * 64x32 cosine matrix used by the MPEG synthesis DCT stage.
 *
 * V[i] = sum(j=0..31)
 *        S[j] * cos((16+i)(2j+1)pi/64)
 *
 * This is the 32-input DCT synthesis operation used to generate the
 * 64 V values required by the ISO polyphase filterbank.
 */
static float gDct32[64][32];
static bool gDct32Init = false;

/*
 * Temporary arrays.  They are reused for every time slot.
 * They are intentionally static so this code does not put large arrays
 * on the ESP32 task stack.
 */
static float gS[32];
static float gU[512];
static float gDwork[576];

/*
 * ISO/IEC 11172-3 Layer III synthesis window, 512 values.
 */
static const float gD[512] = {
...
static inline void dct32Synthesis(const float s[32], float v[64])
{
    /*
     * Explicit 32-input synthesis DCT.
     *
     * Keeping this as a separate function makes the boundary between
     * Hybrid synthesis and the 32-subband polyphase filterbank clear.
     */
    for (unsigned i = 0; i < 64; ++i) {
        float sum = 0.0f;
...
void mp3SynthesisProcess(unsigned channel,
                         ...
                         int16_t pcmOut[576])
{
    if (channel > 1 || !inputHybrid || !pcmOut)
        return;
     ...
     * Frequency inversion.
     *
     * For odd subbands, every odd time sample is inverted before entering
     * the polyphase synthesis bank.
     */
    memcpy(gDwork, inputHybrid, sizeof(gDwork));

    for (unsigned sb = 1; sb < 32; sb += 2) {
        for (unsigned n = 1; n < 18; n += 2) {
            gDwork[sb * 18 + n] =
                -gDwork[sb * 18 + n];
        }
    }
    ...
    /*
     * One MP3 granule contains 18 subband time slots.
     * Every time slot generates exactly 32 PCM samples.
     */
    for (unsigned ss = 0; ss < 18; ++ss) {

        /*
         * Shift the persistent synthesis FIFO by 64 values.
         *
         * The newest 64 V values are written at V[0..63].
         */
        memmove(&gV[channel][64],
                &gV[channel][0],
                (1024 - 64) * sizeof(float));

        /*
         * Gather one time slot:
         *
         *     S[0..31] = hybrid[subband][time_slot]
         */
        for (unsigned sb = 0; sb < 32; ++sb)
            gS[sb] = gDwork[sb * 18 + ss];

        /*
         * 32 -> 64 synthesis DCT.
         */
        dct32Synthesis(gS, gV[channel]);

        /*
         * Build U from the 16 pairs of 32-value blocks required by the
         * MPEG polyphase synthesis matrix.
         *
         * Block starts:
         *
         ...
         *
         * Each selected block contains 32 values.
         */
        for (unsigned block = 0; block < 8; ++block) {
            unsigned u = block * 64;

            memcpy(&gU[u],
                   &gV[channel][block * 128],
                   32 * sizeof(float));

            memcpy(&gU[u + 32],
                   &gV[channel][block * 128 + 96],
                   32 * sizeof(float));
        }

        /*
         * Apply the 512-point synthesis window.
         */
        for (unsigned i = 0; i < 512; ++i)
            gU[i] *= gD[i];

        /*
         * Polyphase accumulation:
         *
         *   PCM[i] = sum(U[i + 32*k]), k=0..15
         *
         * This produces exactly 32 PCM samples for this time slot.
         */
        for (unsigned i = 0; i < 32; ++i) {
            ...
            /*
             * The decoder's hybrid samples are floating point.  Convert
             * the synthesis result to signed 16-bit PCM for the existing
             * MP3PWMOutput path.
             */
            float scaled = sum * 32767.0f;
